...
class AlternatingLeastSquares(MatrixFactorizationBase):
    ...
    def fit(self, item_users, show_progress=True):
        ...
        if self.user_factors is None:
            np.random.seed(self.seed) ; self.user_factors = np.random.rand(users, self.factors).astype(self.dtype)
        if self.item_factors is None:
            np.random.seed(self.seed) ; self.item_factors = np.random.rand(items, self.factors).astype(self.dtype)
